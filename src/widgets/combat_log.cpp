...
inline Element stext_utf8(std::string_view s) { return text(to_wstring(std::string{s})); }
} // namespace

CombatLog::CombatLog(int max_height, size_t max_log_size)
  : max_height_(max_height), max_log_size_(max_log_size), style_map_(DefaultStyleMap()) {}

void CombatLog::Append(std::string_view line_utf8)   { Push(ParseStyledLine(line_utf8)); }
void CombatLog::Append(Element el)                   { Push(std::move(el)); }

void CombatLog::Push(Element el) {
  log_.push_back(std::move(el));
  if (log_.size() > max_log_size_) log_.pop_front();
}

void CombatLog::Clear() { log_.clear(); }
void CombatLog::SetStyle(std::string tag, Decorator deco) { style_map_[std::move(tag)] = deco; }
...
Element CombatLog::Render() {
  ...
  lines.reserve(count);
  for (size_t i = start; i < total; ++i) lines.push_back(log_[i]);
...
Element CombatLog::ParseStyledLine(std::string_view line) const {
  ...
  while (pos < line.size()) {
    ...
    if (tag_start == std::string::npos) { parts.push_back(stext_utf8(line.substr(pos))); break; }
    if (tag_start > pos) parts.push_back(stext_utf8(line.substr(pos, tag_start - pos)));

    size_t tag_end = line.find("](", tag_start);
    if (tag_end == std::string::npos) { parts.push_back(stext_utf8(line.substr(tag_start))); break; }

    const size_t value_open = tag_end + 2;
    size_t val_end = line.find(')', value_open);
    if (val_end == std::string::npos) { parts.push_back(stext_utf8(line.substr(tag_start))); break; }
