...
public:
  ...
  Engine& eng() const {
    struct Slot { bool init{false}; Engine e; };
  ...
  template <typename T>
  void shuffle(std::vector<T>& v) { std::shuffle(v.begin(), v.end(), eng()); }

  template <typename Range>
  const auto& pick(const Range& r) {
  ...
  // Burn n draws:
  void advance(uint64_t n) { for (uint64_t i = 0; i < n; ++i) (void)u64(); }
...
private:
  ...
  uint64_t sequence_{1};        // pcg allows any; constructor handles odd increment internally
...
class RandomHub {
public:
  // If seed == nullopt → time-based; base_sequence is an optional global “stream id”.
  explicit RandomHub(std::optional<uint64_t> seed = std::nullopt, uint64_t base_sequence = 0)
  ...
  // Named stream: sequence selector derived from base_sequence ^ hash(name) ^ sub_seq
  ...
  RandomStream stream(uint64_t sub_seq) const {
    const uint64_t seq = mix_seq(base_sequence_, 0xD1B54A32D192ED03ULL, sub_seq);
  ...
  class NS {
  public:
    NS(const RandomHub& hub, std::string prefix) : hub_(hub), prefix_(std::move(prefix)) {}
    NS ns(std::string_view child) const { return NS{hub_, prefix_ + "/" + std::string(child)}; }
    NS ns(std::string_view child, uint64_t id) const {
      return NS{hub_, prefix_ + "/" + std::string(child) + "/" + std::to_string(id)};
    }
    RandomStream stream(uint64_t sub_seq = 0) const { return hub_.stream(prefix_, sub_seq); }
...
private:
  static uint64_t mix_seq(uint64_t base, uint64_t name_hash, uint64_t sub) {
    ...
    return splitmix64_once(base ^ (name_hash + 0x9E3779B97F4A7C15ULL) ^ (sub * 0xBF58476D1CE4E5B9ULL));
  }

  static std::string key_for(std::string_view name, uint64_t sub) {
    if (sub == 0) return std::string(name);
