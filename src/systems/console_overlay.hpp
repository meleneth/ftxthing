...
public:
  ...
  ftxui::Element Render() override {
    ...
    auto log_window =
        //      window(text(" Console "), vbox(std::move(rows)) | yflex) |
        //      size(HEIGHT, GREATER_THAN, 12);
        window(text(" Console "), vbox("foo") | yflex) |
        size(HEIGHT, GREATER_THAN, 12);
    ...
    auto overlay = vbox({log_window, bar}) | bgcolor(ftxui::Color::Black) |
  ...
  bool OnEvent(ftxui::Event e) override {
    ...
    if (e == ftxui::Event::ArrowUp) {
      if (hist_pos_ > 0) {
        --hist_pos_;
        line_ = history_[hist_pos_];
      }
      return true;
    }
    if (e == ftxui::Event::ArrowDown) {
      if (hist_pos_ + 1 < history_.size()) {
        ++hist_pos_;
        line_ = history_[hist_pos_];
      } else {
        hist_pos_ = history_.size();
