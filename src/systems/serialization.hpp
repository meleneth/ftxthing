...
struct SaveV1 {
  ...
  auto serialize(Archive& ar) { return ar(seed, player, inventory_ids); }
};
...
struct SaveV2 { // later addition: difficulty and map seed
  ...
  uint64_t map_seed{};
  template <typename Archive>
  auto serialize(Archive& ar) { return ar(seed, player, inventory_ids, difficulty, map_seed); }
};

// --- Writing (to bytes) ---
template <class T>
std::vector<std::byte> save_to_bytes(const T& s) {
