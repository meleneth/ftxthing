...
struct ev_tab_next {};
struct ev_tab_prev {};
struct ev_open_party { int party_id; };
struct ev_open_override { int screen_kind; };
...
struct UiMachine {
  ...
  void show_current() const {
    if (c.show_screen) c.show_screen(c.current_index);
  }

  // ---- actions ----
  auto next_screen() const {
    return [this] {
      if (c.screen_count <= 0) return;
  ...
  auto open_party() const {
    return [this](const ev_open_party& e) {
      if (c.open_party_screen) c.open_party_screen(e.party_id);
    };
  }
  auto open_override() const {
    return [this](const ev_open_override& e) {
      if (c.push_override) c.push_override(e.screen_kind);
    };
  }
  auto toggle_console() const {
    return [this] {
      if (!c.set_console_open || !c.is_console_open) return;
      c.set_console_open(!c.is_console_open());
    };
  }
  auto close_one() const {
    return [this] {
      if (c.has_override && c.has_override()) { if (c.pop_override) c.pop_override(); return; }
      if (c.is_console_open && c.is_console_open()) { if (c.set_console_open) c.set_console_open(false); return; }
      // nothing to close
    };
  }
  ...
  auto operator()() const {
    ...
    return make_transition_table(
      *state<Active> + event<ev_tab_next>       [has_screens() && console_closed()] / next_screen() = state<Active>,
       state<Active> + event<ev_tab_prev>       [has_screens() && console_closed()] / prev_screen() = state<Active>,
       state<Active> + event<ev_open_party>                                      / open_party()     = state<Active>,
       state<Active> + event<ev_open_override>                                   / open_override()  = state<Active>,
       state<Active> + event<ev_toggle_console>                                  / toggle_console() = state<Active>,
       state<Active> + event<ev_esc>                [console_open() || overlay_present()] / close_one()      = state<Active>
    );
  }
};
