...
TrackXP::TrackXP(int starting_xp)
    : xp_(starting_xp), next_level_at(xp_for_level(level_ + 1)) {}

void TrackXP::add_xp(fairlanes::context::EntityCtx &ctx, int amount) {
  xp_ += amount;
  using fairlanes::ecs::components::Stats;
  while (next_level_at && xp_ >= next_level_at) {
    ++level_;
    next_level_at = xp_for_level(level_ + 1);
    if (auto info = ctx.reg_->try_get<Stats>(ctx.self_)) {
      ctx.log_->append_markup(fmt::format(
